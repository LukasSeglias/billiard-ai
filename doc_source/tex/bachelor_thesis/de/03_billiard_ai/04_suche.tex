...
Durch Studium des Beispiels eines Stosses über zwei Banden wird ein Algorithmus hergeleitet, wobei die Mehrdimensionalität vernachlässigt
...
In der Abbildung \ref{fig:zweifaches_bandenspiel_c} wird der darauffolgende Schritt gezeigt. Es gibt zum Vorhergehenden
...
Diese Spiegelung überführt den Punkt $\bar{C_0}$ auf den ursprünglichen Punkt $C$, was auf das Ende des Algorithmus
hindeutet.
\begin{figure}[h!]
    \begin{center}
...
Dasselbe Prinzip kann auch auf ein Spiel über drei Banden angewendet werden. Die Abbildung \ref{fig:Dreifache Reflektion an Banden} veranschaulicht
ein Solches. Es wird zuerst an der linken, dann an der oberen und anschliessend an der rechten Bande gespiegelt.
Es wird ein einem Schritt wiederum der letze Spiegelpunkt und dessen Startposition betrachtet, mit welchen der Kollisionspunkt
an einer Bande gefunden werden kann. Danach kann der Spiegelpunkt an dieser Bande gespiegelt werden und der nächste
Schritt kann starten. Dies wird solange fortgesetzt, wie es Banden zum spiegeln hat.
\begin{figure}[h!]
    \begin{center}
