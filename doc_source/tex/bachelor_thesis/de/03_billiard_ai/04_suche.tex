...
\begin{algorithm}[H]
    ...
    \Fn{\combos{reflections: int, rails: Rail[]} $\longrightarrow$ Rail[][]}{
        combinations: Rail[][] $\longleftarrow$ []\\
        lastReflection: vec2[] $\longleftarrow$ []\\
        \For{rail in rails}{
            combinations $\longleftarrow$ append([rail], combinations)\\
            lastReflection $\longleftarrow$ append($(0, 0)$, lastReflection)
        }
        \For{index = 0 < reflections}{
            \For{railIndex $\longleftarrow$ 0 < length(rails)}{
                rail $\longleftarrow$ rails[railIndex]\\
                \If{canReflect(lastReflection[railIndex], rail)}{
                    combinations[railIndex] $\longleftarrow$ append(rail, combinations[railIndex])\\
                    lastReflection[railIndex] $\longleftarrow$ reflect(lastReflection[railIndex], [rail])[0]
                }
            }
        }
...
\begin{algorithm}[H]
    ...
    \Fn{\reflect{C: vec2, rails: Rail[]} $\longrightarrow$ vec2[]}{
        ...
        \For{rail in Rail}{
            ...
            reflections $\longleftarrow$ append(reflected, reflections)\\
